...
돈이 걸린 시스템이라 "두 번 처리됨", "잔액이 음수가 됨", "돈이 묶인 채 사라짐" 이 전부 사고다. 이 저장소의 대부분은 그 사고를 하나씩 막는 장치이고, **그 장치가 무엇이고 어디까지 검증됐는지**는 [`docs/SYSTEM.md`](docs/SYSTEM.md) 한 장에 정리돼 있다.
...
결제 없는 자기 충전은 없다(실제 결제 충전은 [제품 방향](docs/SYSTEM.md#10-제품-방향) step14). 그전까지 크레딧은 **운영자 지급**으로만 생긴다. 브라우저에서는 운영자로 로그인해 `/admin` 화면의 지급 폼을 쓴다.
...
**문서는 하나다.** [`docs/SYSTEM.md`](docs/SYSTEM.md) — 흐름 6단계, 돈을 지키는 장치 셋,
회수 정책, 실측 결과와 판정, 알려진 결함 셋, 설계 결정 이력, 배포 전 확인, 다음에 할 일.

| | |
|---|---|
| [`docs/SYSTEM.md`](docs/SYSTEM.md) | **정본.** 이것만 읽으면 된다 |
| [`load/README.md`](load/README.md) | 부하 측정 실행 절차와 함정 |
| [`deploy/observability/README.md`](deploy/observability/README.md) | 관측 스택 · 장애 주입 시나리오 8개 |

2026-09-23 에 문서를 통폐합했다. 그전에는 현황·요구서·설계·체크리스트·결과와 ADR 7편,
단계별 학습 문서 11편, 측정 결과 5편이 따로 있었다. 전부 `docs/SYSTEM.md` 로 접었고
원본은 git 이력에 남아 있다 — 태그 `req-v0`(현황) · `req-v1`(요구서) · `req-v2`(설계·ADR) ·
`req-v3`(검증·결과)에서 그 시점의 전체 문서를 볼 수 있다.
...
저장소에 평문 비밀번호는 없지만, compose 의 환경변수는 여전히 평문이다. 진짜 시크릿 관리는 배포 환경이 정해질 때([제품 방향](docs/SYSTEM.md#10-제품-방향) step10) 붙는다.

---

## 알아 둘 것

- **로그인할 수 있는 사람은 허용 목록뿐이다.** 공개 가입이 없다. 실사용자가 나 한 명인 서비스다([제품 방향](docs/SYSTEM.md#10-제품-방향) 결정 9)
- **크레딧은 운영자 지급으로만 생긴다.** 결제 충전은 step14 다([제품 방향](docs/SYSTEM.md#10-제품-방향))
- **생성이 아직 스텁이다.** `GenerationClient` 포트는 step11 에서 생겼지만, 그 자리에 든 구현은 `Thread.sleep` + 확률적 실패인 인메모리 시뮬레이션이다. 진짜 Claude 는 step12 다([제품 방향](docs/SYSTEM.md#10-제품-방향))
...
- **인스턴스 1대 전제다.** 회수·대사·멱등키 정리 스케줄러에 분산 락이 없다. 실사용자가 한 명이라 서버도 1대로 운영하기로 했다([제품 방향](docs/SYSTEM.md#10-제품-방향) 결정 13)
- **내가 실제로 쓰는 개인 서비스로 가는 중이다.** 원화로 앱 내 크레딧을 충전(mock)하고, 그 크레딧으로 Claude API 요청을 산다([제품 방향](docs/SYSTEM.md#10-제품-방향), 원본은 로드맵 v4). 돈이 걸린 시스템에서 무엇이 깨지고 무엇으로 막는지를 코드와 실측으로 남기는 원칙은 그대로다
