...
결제 없는 자기 충전은 없다(실제 결제 충전은 [로드맵](`docs/SYSTEM.md`) step14). 그전까지 크레딧은 **운영자 지급**으로만 생긴다. 브라우저에서는 운영자로 로그인해 `/admin` 화면의 지급 폼을 쓴다.
...
- **로그인할 수 있는 사람은 허용 목록뿐이다.** 공개 가입이 없다. 실사용자가 나 한 명인 서비스다([로드맵](`docs/SYSTEM.md`) 결정 9)
