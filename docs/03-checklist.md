...
- [x] **PERF-03** 외부 지연 무관성 — **부분 통과**. 외부를 60배(50~100ms → 3~7초) 느리게 해도
  ...
  (`...scenarios-02-03-04-and-capacity.md` 3절).
- [x] **PERF-04** 원장 1억 행에서 최근 내역 조회 — **측정 완료**(2026-09-23,
  `load/results/2026-09-23-perf04-large-ledger.md`). 워밍·스케줄러 끔에서 p99 10.13/12.68ms 로
  목표(50ms) 안. **단 스케줄러가 도는 운영 기본값에서는 p99 8,489ms** 다 — 아래 대사 항목을 볼 것
- [ ] **대사 배치가 1억 행에서 완료된다** — **실패**. `LIMIT 100` 이 `GROUP BY` 뒤에 걸려
  100명을 얻으려 1억 행을 전부 집계한다. 265초 지나도 미완료였고 앱 로그에 완료 기록이 0건이다.
  `app.scheduling.reconciliation-interval-millis` 60초보다 한 주기가 길다.
  **배포 전 이 쿼리를 고치거나 대사 주기를 조정해야 한다**
- [x] 인덱스가 충분한지 판정됐다 — `idx_ledger_user_id (user_id)` 단일 컬럼으로 충분하다.
  1억 행 `EXPLAIN` 이 첫 페이지에 `Backward index scan`(filesort 없음). 복합 인덱스 불필요
