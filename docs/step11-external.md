...
backoff 가 job 당 최대 +50초(10 + 40)를 더한다. 종결을 기다리는 시나리오(06 의 180초, 01 의 240초)가 그 아래에서도 충분한지 재실행으로 확인했다 — **둘 다 여유가 있었다**(01 은 SIGKILL 후 15초에 회수, 06 은 예산 안에서 끝났다). 상한은 손대지 않았다.

## 시나리오 재실행 (2026-09-20)

`run-all.sh` 한 번으로 01~08 을 전부 돌렸다(약 50분). 첫 시도는 03 진행 중 외부에서 끊겼고, 두 번째 실행이 끝까지 갔다.

| # | 이전 (step9 재실측 / step7) | 이번 | 판정 |
|---|---|---|---|
| 01 워커 크래시 | `recovery{heartbeat}` 3 / SIGKILL 후 14초(앱 UP 후 4초), `backstop` 0, 알람 없음 | 3 / SIGKILL 후 14초(앱 UP 후 4초), `backstop` 0, `retry_claim` 3, 알람 없음, 불변식 0 | **같음** |
| 02 A ZSET 소실 | `backstop` 3 (앱 UP 후 53초), `heartbeat` 0, `backstop_blind` 0 | 3 (51초 / SIGKILL 후 63초), 0, 0. `CreditBackstopRecovery` +4초 | **같음** |
| 02 B Redis 다운 | `backstop_blind` 3 (+8초), WARN 3회, 복구 후 추가 회수 없음, 워커 재개 후 7초 `COMPLETED=7` | 3 (+14초), WARN 3회·ERROR 9회, 추가 회수 없음, 6초 `COMPLETED=7` | **같음** |
| 03 워커 정지 | count 5 / amount 500, `CreditPipelineStalled` 383초 | 5 / 500, 373초, 사고 관련 카운터 0 | **같음** |
| 04 스케줄러 정지 | staleness -1, 두 Stale 알람 firing, 게이지 0 에 얼어붙음(DB 121초) | 같음(DB 121초). 방어 카운터 합 3 | **같음** |
| 05 원장 훼손 | mismatch 39초 / negative 12초 / jobs_without_hold 12초, 증가 0 | 27초 / 12초 / 8초, 증가 0(18 → 18), 원복 후 알람 0 | **같음** |
| 06 중복 폭풍 | `app_hit` 90 + `db_unique` 9 = 99, 200 91·409 9, 잔액 -100, job 1행 | 99(`app_hit` 93 + `db_unique` 6), 잔액 -100, job 1행 | **같음** (앱 조회와 DB 유니크의 분배는 경합 타이밍에 따라 달라진다. 합이 99 인 것이 판정 대상이다) |
| 07 외부 지연 → 타임아웃 | step7 에서는 **끝나지 않았다**(회수 0, `worker_claim` 3 에서 멈춤) | **138초에 종결.** `mark_failed` 9 / `retry_claim` 6 / `final_refund` 3, 잔액 10000 원복, 슬롯 3 복귀, `hard_cap` 0 | **달라졌다 — 11-A 가 산 것** |
| 08 멈춘 워커 (새 시나리오) | — | `hard_cap` 3, `final_refund` 1, 잔액 10000 원복(**돈은 풀린다**), `slots_free` **0 그대로**, 고아 ZSET 3, 새 job 은 `HOLDING` 에 갇힘, `CreditHardCapRecovery`·`CreditWorkerSlotsExhausted` 둘 다 firing | **설계대로**(아래 주 참고) |

**07 이 이 단계의 요약이다.** 같은 사고(600초 지연)를 step7 때와 똑같이 심었는데, 그때는 job 이 `PROCESSING` 에서 끝나지 않았고 지금은 138초에 환불까지 끝난다. 그 차이가 타임아웃 한 줄이다.

**08 의 주.** 예상 타임라인은 환불까지 약 335초였으나 실측은 **549초**였다(절대 상한 90초 × 3회 + backoff 10·40초 + 스캔·스크레이프 지연). 차이의 원인은 분석하지 않았다 — 판정 대상은 "회수가 되고 돈이 풀리는가"이고 그건 그대로다. 슬롯은 시도마다 하나씩 묶여 3 → 2 → 1 → 0 으로 줄었고 **돌아오지 않았다.** 사고 직후 `T+0 사고 전 빈 슬롯 = 0` 으로 찍힌 줄은 첫 스크레이프 전에 조회한 측정 artifact 이고 실제 값이 아니다. 슬롯이 0 인 상태에서 새로 넣은 job 은 45초 뒤에도 `HOLDING` 이었다 — **돈은 풀렸지만 파이프라인은 멈춘다.** 복구는 재기동뿐이다.
