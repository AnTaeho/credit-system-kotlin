...
| 태우는 속도 | 제한 없음 | 인증이 뚫리거나 내 스크립트가 루프를 돌면 선결제 크레딧이 요청 속도 그대로 탄다 |
...
**D — 사용자별 job 접수 속도 제한 (`eab84fb`)**

결정 9·12 의 "나 자신의 실수"를 막는 벽이다. `POST /api/jobs` 만 건다 — 돈을 묶는 유일한 사용자 경로다.

- 사용자별 토큰 버킷. 용량 = 분당 허용 수(기본 10), 1분에 걸쳐 균등하게 다시 찬다. 서버 1대(결정 13)라 카운터는 앱 메모리다
- **컨트롤러 첫 줄에서 `HoldService` 보다 먼저** 검사한다. 거절된 요청은 원장·job·멱등키·잔액 어디에도 흔적이 없다
- 초과는 429 `RATE_LIMITED` + `Retry-After`(초, 올림, 최소 1)
- 방어 지표에 `point="rate_limit"`(`applied`/`rejected`)을 더했다. `userId` 태그는 달지 않는다(카디널리티)
- 토큰을 소수가 아니라 정수 단위로 센다. "정확히 6초 뒤 1개" 같은 경계에서 0.9999… 가 나와 거절이 한 번 더 붙는 것을 피한다
...
181 → **291**. 조각별로 A 181(그대로) → B 215 → C 235 → E 250 → 후속 251 → D 266 → F 291.
...
- **속도 제한:** 50 스레드 동시 요청에도 정확히 용량만큼만 통과, 거절된 요청은 흔적 없음, `Retry-After` 계산
...
- `docker-compose.yml` 의 app 에 `SPRING_PROFILES_ACTIVE: local`(개발 로그인·허용 목록)과 `APP_RATELIMIT_JOBCREATE_PERMINUTE: ${…:-100000}`
...
**속도 제한을 푸는 이유.** 시나리오는 한 사용자로 job 을 몰아서 만들고, 06 은 같은 `idemKey` 로 100건을 동시에 던진다. 제한이 분당 10 이면 90건이 429 로 먼저 끝나서 step7 에서 잰 `idem_key{app_hit}+{db_unique}=99` 가 다른 이유로 깨진다. 관측 스택은 방어 지표를 보는 곳이지 속도 제한을 보는 곳이 아니라서, compose 에서 크게 풀었다.

환경변수 이름이 먹는지는 직접 확인했다. 속성 `app.rate-limit.job-create.per-minute` 은 Spring 완화 바인딩에서 대시가 빠져 `APP_RATELIMIT_JOBCREATE_PERMINUTE` 이 된다.

- 기본(100000)으로 띄운 smoke: job 요청 13건이 전부 속도 제한을 통과했다(200 11건, 잔액 부족 409 2건 — 분당 10 이면 11번째부터 429 였다). `rate_limit{applied}=13`, `{rejected}=0`
- `APP_RATELIMIT_JOBCREATE_PERMINUTE=2` 로 앱만 다시 만들고 4건: 409, 409, **429, 429** — 3번째부터 `RATE_LIMITED`

대시보드는 고치지 않았다. 방어 패널이 `sum by (point, outcome)` 이라 `rate_limit` 이 자동으로 한 줄 더 생긴다. `point` 를 명시적으로 나열하는 패널은 `idem_key` 하나뿐이다. 알람 규칙도 그대로다.
...
| 06 중복 폭풍 | `app_hit` 90 + `db_unique` 9 = 99, HTTP 200 91·409 9, 잔액 -100, job 1행 | 90 + 9 = 99, 200 91·409 9(**429 없음**), -100, 1행 | 같음 |
| 07 외부 API 지연 | `CreditPipelineStalled` 379초, 회수 0, `worker_claim` 정확히 3 | 383초, 0, 3 | 같음 |

1. **04 의 "방어 카운터 합"이 3 이 아니라 6 이다.** job 3건 접수가 `hold_balance/applied` 3 에 더해 `rate_limit/applied` 3 을 남긴다(D 가 접수마다 한 번 센다). 스크립트의 기대값 문구가 step7 시점의 "job 생성분 3" 이라 "job 생성분 6" 으로 고쳤다. 판정 대상(스케줄러가 멈춰도 방어 카운터는 job 생성분만 움직인다)은 그대로다
...
- **브라우저 JS 를 실제 브라우저로 돌려 보지 않았다.** 화면의 폴링·재로그인·멱등키 재사용은 서버 쪽 테스트, curl 로컬 스모크, `node --check` 구문 검사까지만 봤다
...
- **속도 제한 카운터는 앱 메모리다.** 재시작하면 버킷이 가득 찬 채로 돌아온다. 서버 1대(결정 13)라 받아들였다
- **Redis 가 죽으면 health 가 DOWN 이다.** 위 CI 절. step10 의 health 재설계
- **관측 스택은 속도 제한을 끈 것과 같다.** 시나리오가 속도 제한 자체를 실측하지는 않는다
...
뒤의 둘은 속성 이름의 대시가 빠진다(`ALLOWED_EMAILS` 가 아니다). Spring 완화 바인딩이 환경변수 이름을 만드는 규칙이다 — 점은 밑줄, 대시는 제거, 대문자. 같은 규칙으로 `APP_RATELIMIT_JOBCREATE_PERMINUTE`, `APP_ADMIN_MAXGRANTAMOUNT` 도 쓸 수 있다.
