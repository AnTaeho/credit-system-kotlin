...
- ~~브라우저·구글 로그인 미검증~~ — 2026-09-20 둘 다 확인했다. 개발 로그인으로 화면 한 바퀴(로그인 → 운영자 지급 → 요청 → 폴링 상태 갱신 → 원장, 서버 로그에 지급 1·확정 9·실패 2·재시도 2), 그리고 실제 구글 계정으로 로컬 로그인(`첫 로그인 사용자 생성: userId=3, email=a***@gmail.com` — 허용 목록 통과, 이메일은 마스킹되어 기록)
