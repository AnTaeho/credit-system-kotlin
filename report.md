...
> **2026-08-25 갱신: 이 절의 평탄화는 되돌렸다(`d619e48`).** 패키지 구분은 계층을 드러내는
> 장치인데 그걸 없애고 파일 수가 준 것을 가독성이 좋아진 것으로 착각했다. Q-1(RedisOutageGate
> 흡수)과 Q-4(JPQL 함정)는 그대로 유효하다 — 되돌린 것은 Q-2·Q-3 의 패키지 평탄화와 파일 병합이다.
> 판단 착오의 내용은 S 절 참고.
...
## S. 패키지 평탄화를 되돌렸다 — 무엇을 단순화해야 했나 (2026-08-25)

Q 의 평탄화를 통째로 되돌렸다(`d619e48`). 무엇이 틀렸는지 남겨둔다.

### S-1. 착각한 것

"흐름에서 뭘 읽어야 할지 안 보인다"는 문제에 **파일 수와 패키지 수를 세어** 답했다.
1700줄 / 52파일 / 25패키지라는 숫자를 보고 "비계가 많다"고 진단한 뒤, 파일을 합치고
`ledger`·`organization`·`global` 의 하위 패키지를 없앴다.

**숫자가 준 것과 읽기 쉬워진 것은 다른 문제였다.** 패키지 구분은 계층을 드러내는 장치다.
`ledger/repository/LedgerRepository.kt` 는 경로만으로 "원장 도메인의 영속성 계층"이라고
말해주는데, `ledger/LedgerRepository.kt` 는 그 말을 파일 이름에만 맡긴다. 디렉터리를 열지
않아도 되는 것과 어디에 무엇이 있는지 아는 것은 같은 게 아니다.

`파일이 6개 이하인 도메인은 평탄화한다`는 규칙도 근거가 약했다. 도메인의 **파일 개수**는
그 도메인에 계층이 있는지 없는지와 아무 상관이 없다.

### S-2. 요청은 그게 아니었다

요구는 "패키지 구분을 없애라"가 아니라 **"흐름에 맞게 패키지가 잘 나뉘고, 코드가 단순하고,
연관관계가 깔끔해져라"** 였다. 단순화의 대상은 **코드와 의존 관계**지 디렉터리 구조가 아니다.

이 세션에서 실제로 값어치가 있었던 것들이 그 증거다 — 전부 코드와 의존을 건드린 것이지
구조를 지운 게 아니다:

| 한 일 | 무엇을 단순화했나 |
|---|---|
| N. `scheduler` 해체 | `job ↔ scheduler` **순환 의존** 제거 |
| O. `HeartbeatProperties` 독립 | heartbeat 가 앱 전역 설정 덩어리를 모르게 됨 |
| P. 억제 경보 기계 제거 | `AtomicReference` 3개 + CAS → 1개 |
| Q-1. `RedisOutageGate` 흡수 | 존재 이유가 사라진 클래스 하나 |
| R. 중복 쿼리 → default 메서드 | 같은 JPQL 두 벌 → 한 벌 |

반면 Q-2·Q-3(패키지 평탄화, 파일 병합)은 **아무 의존도 정리하지 않았다.** 선언을 옮겨
담기만 했다. 그게 값어치가 없었던 이유다.

### S-3. 되돌린 범위

- `ledger`, `organization`, `global` 의 하위 패키지 복원
- 합쳤던 파일 분리 (`Exceptions.kt` → 5개, `*Dtos.kt` → 도메인별로, `JobStatus`·`LedgerType` 등)
- `job/controller`, `job/repository`, `job/stub` 복원

**남긴 것**: Q-1 의 `RedisOutageGate` 흡수는 그대로다. 그건 파일 병합이 아니라 존재 이유가
사라진 클래스를 없앤 것이라 성격이 다르다.

평탄화 이후에 얹힌 R 의 쿼리 작업도 그대로 살아 있다. `LedgerBalanceCheck` 의 FQ 이름은
`ledger.dto` 로 함께 돌아가 Q-4 의 함정과 아귀가 맞는다.

### S-4. 남는 기준

**구조를 세는 것으로 복잡도를 진단하지 않는다.** 파일 수·패키지 수는 증상도 원인도 아니다.
다음에 "복잡하다"는 말이 나오면 물어야 할 것은 이쪽이다 — 어떤 의존이 방향을 거스르는가,
어떤 클래스가 존재 이유 없이 서 있는가, 같은 일을 하는 코드가 몇 벌인가.

`./gradlew cleanTest test ktlintCheck detekt` → 126 테스트, 실패 0, 에러 0 (되돌린 뒤에도 그대로).
