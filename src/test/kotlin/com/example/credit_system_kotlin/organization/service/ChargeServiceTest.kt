...
@ActiveProfiles("test")
@DataJpaTest
class ChargeServiceTest @Autowired constructor(
    private val organizationRepository: OrganizationRepository,
    private val ledgerRepository: LedgerRepository
) {

    private val chargeService = ChargeService(organizationRepository, ledgerRepository)

    @Test
    fun `충전하면 잔액이 증가하고 ledger에 CHARGE가 남는다`() {
        val organization = organizationRepository.save(Organization("acme", 500L))

        val response = chargeService.charge(organization.persistedId, "idem-1", 300L)
    ...
    fun `같은 idemKey로 두 번 충전해도 잔액은 한 번만 오른다`() {
        ...
        val first = chargeService.charge(organization.persistedId, idemKey, 300L)
        val second = chargeService.charge(organization.persistedId, idemKey, 300L)
    ...
    fun `다른 idemKey면 각각 충전된다`() {
        ...
        val first = chargeService.charge(organization.persistedId, "idem-a", 300L)
        val second = chargeService.charge(organization.persistedId, "idem-b", 200L)
    ...
    fun `idemKey가 공백이면 거부한다`() {
        ...
        assertThatThrownBy { chargeService.charge(organization.persistedId, "   ", 300L) }
    ...
    fun `idemKey가 100자를 초과하면 거부한다`() {
        ...
        assertThatThrownBy { chargeService.charge(organization.persistedId, tooLong, 300L) }
            .isInstanceOf(InvalidRequestException::class.java)
    ...
    fun `충전 금액이 0 이하면 거부한다`() {
        ...
        assertThatThrownBy { chargeService.charge(organization.persistedId, "idem-1", 0L) }
            .isInstanceOf(InvalidRequestException::class.java)
    ...
    fun `충전 금액이 상한을 초과하면 거부한다`() {
        ...
        assertThatThrownBy { chargeService.charge(organization.persistedId, "idem-1", 1_000_001L) }
            .isInstanceOf(InvalidRequestException::class.java)
